...
bool Win32Filesystem::DeleteFile(const Pathname &filename) {
  ...
  return ::DeleteFile(filename.pathname().c_str()) != 0;
...
//    return ToUtf8(filename);
  ...
  if (::GetTempFileName(dir.pathname().c_str(),
                        prefix.c_str(), 0, filename) != 0)
    return filename;
  RTC_NOTREACHED();
  return "";
}
...
bool Win32Filesystem::MoveFile(const Pathname &old_path,
  ...
  return ::MoveFile(old_path.pathname().c_str(),
                    new_path.pathname().c_str()) != 0;
...
bool Win32Filesystem::IsFolder(const Pathname &path) {
  ...
  if (0 == ::GetFileAttributesEx(path.pathname().c_str(),
...
bool Win32Filesystem::GetFileSize(const Pathname &pathname, size_t *size) {
  WIN32_FILE_ATTRIBUTE_DATA data = {0};
  if (::GetFileAttributesEx(pathname.pathname().c_str(),
